...
echo "Start do repo sync & build atrix  `date`">>$dobuildlog
...
breakfast olympus  >>$dobuildlog 2>&1     # for atrix
time make -j3 bacon   >>$dobuildlog 2>&1  # j3 is number of cores
##. build/envsetup.sh && brunch model_name
echo "End do repo sync & build atrix  `date`">>$dobuildlog
